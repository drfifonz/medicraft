...
if __name__ == "__main__":
    ...
    DATASET_PATH = ".results/datasets/0004"
    ...
    # print(ONEDRIVE_DESTINATION_DIR)
    # raise
    ...
    model_name, model_path = current_model_name, models[current_model_name]

    diffusion.load_state_dict(torch.load(model_path)["model"], strict=False)
    checkpoint = torch.load(model_path)  # TODO unused variable ?
    dataset_path = Path(DATASET_PATH) / model_name
    Path.mkdir(dataset_path, parents=True, exist_ok=True)
    print(f"Model {model_name} loaded from {model_path}.")
    print(f"Generating samples for {model_name} in range {START_SAMPLE_IDX}-{START_SAMPLE_IDX+1000}.")
